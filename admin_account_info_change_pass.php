<?php 
    ...
    if (isset($_POST['submit'])) {
        ...
        $c_pass = $_POST['c_pass'];
        if ($new_pass == $c_pass) {
            $sql = "SELECT * FROM ADMIN_ACCOUNT WHERE acc_num='$accnum' AND PASS='$old_pass'";
            $result = mysqli_query($conn, $sql);
            if ($result->num_rows > 0) {
                $sql = "UPDATE ADMIN_ACCOUNT SET PASS='$new_pass' WHERE acc_num='$accnum' ";
                $result = mysqli_query($conn, $sql);
                if ($result) {
...
<head>
    ...
    <title>Admin Account Information Change Password</title>
</head>
<body>
