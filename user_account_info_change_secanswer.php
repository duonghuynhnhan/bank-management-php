<?php 
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($new_ans == $c_ans) {
            $sql = "SELECT * 
                    FROM USER_ACCOUNT 
                    WHERE acc_num='$accnum' AND ANSWER='$old_ans'";
            $result = mysqli_query($conn, $sql);
            if ($result->num_rows > 0) {
                $sql = "UPDATE USER_ACCOUNT SET ANSWER='$new_ans' WHERE acc_num='$accnum' ";
                $result = mysqli_query($conn, $sql);
                if ($result) {
...
<head>
    ...
    <title>User Account Information Change Answer</title>
</head>
<body>
